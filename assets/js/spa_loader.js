document.addEventListener('DOMContentLoaded', () => {
    ...
    function getCurrentPageFromHash() {
        ...
        if (hash) {
            // Split the hash into parts, e.g., "#/github.html#pi-cow-garage" -> ["", "github.html", "pi-cow-garage"]
            const hashParts = hash.split('#');
            const pageWithExt = hashParts[1] ? hashParts[1].replace('/', '') : null;
            
            if (pageWithExt) {
                const pageName = pageWithExt.split('.')[0]; // "github"
                const anchor = hashParts[2] || null; // "pi-cow-garage"
                return { pageName, anchor };
            }
        }
        return { pageName: 'home', anchor: null }; // Default page and no anchor
    }
    ...
    async function loadContent(url, pageName, anchor = null, isInitialLoad = false) {
        try {
            ...
            // The content is now in the DOM. Call the social share init function.
